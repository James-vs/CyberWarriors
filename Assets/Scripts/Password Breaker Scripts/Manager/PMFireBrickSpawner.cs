...
public class PMFireBrickSpawner : MonoBehaviour
{
    // variable to count collisions for stronger brick types
    //private float collisionCount = 5f;
    //public GameObject dynamicFirewall;
    public GameObject eventSystem;

    // Start is called before the first frame update
    void Start()
    {
        Debug.Log("PMBallShooter script start");
    ...
    public void OnCollisionEnter2D(Collision2D other) {
        ...
        if (collisionCount >= 5f) {
            ...
            Instantiate(dynamicFirewall, new Vector3(transform.position.x - 3f, transform.position.y, transform.position.z), transform.rotation);
